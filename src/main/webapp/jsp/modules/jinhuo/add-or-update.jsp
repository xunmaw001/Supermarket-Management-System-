                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="gongyingshangId" name="gongyingshangId" type="hidden">
                                    <div class="form-group col-md-6 jinhuoUuidNumberDiv">
                                        <label>进货单号</label>
                                        <input style="width: 450px" id="jinhuoUuidNumber" name="jinhuoUuidNumber" class="form-control"
                                               placeholder="进货单号">
                                    </div>
                                    <div class="form-group col-md-6 jinhuoNumberDiv">
                                        <label>进货数量</label>
                                        <input style="width: 450px" id="jinhuoNumber" name="jinhuoNumber" class="form-control"
                                               onchange="jinhuoNumberChickValue(this)"  placeholder="进货数量">
                                    </div>

                                    <div class="form-group col-md-6 jinhuoTimeDiv">
                                        <label>进货时间</label>
                                        <input style="width: 450px" id="jinhuoTime-input" name="jinhuoTime" type="text" class="form-control layui-input">
                                    </div>
                                    <div class="form-group  col-md-6 jinhuoContentDiv">
                                        <label>进货备注</label>
                                        <input id="jinhuoContentupload" name="file" type="file">
                                        <script id="jinhuoContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var jinhuoContentUe = UE.getEditor('jinhuoContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="jinhuoContent" id="jinhuoContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#jinhuoContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('jinhuoContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("jinhuo/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addjinhuo');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.jinhuoContent != null && ruleForm.jinhuoContent != 'null' && ruleForm.jinhuoContent != '' && $("#jinhuoContentupload").length>0) {

            var jinhuoContentUeditor = UE.getEditor('jinhuoContentEditor');
            jinhuoContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.jinhuoContent != null){
                    mes = ''+ ruleForm.jinhuoContent;
                    // mes = mes.replace(/\n/g, "<br>");
                }
                jinhuoContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#jinhuoContentupload").length>0) {
            var jinhuoContentEditor = UE.getEditor('jinhuoContentEditor');
            if (jinhuoContentEditor.hasContents()) {
                $('#jinhuoContent-input').attr('value', jinhuoContentEditor.getContent());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function jinhuoNumberChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                jinhuoUuidNumber: "required",
                jinhuoNumber: "required",
                jinhuoTime: "required",
                jinhuoContent: "required",
            },
            messages: {
                yuangongId: "员工不能为空",
                shangpinId: "商品不能为空",
                gongyingshangId: "供应商不能为空",
                jinhuoUuidNumber: "进货单号不能为空",
                jinhuoNumber: "进货数量不能为空",
                jinhuoTime: "进货时间不能为空",
                jinhuoContent: "进货备注不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addjinhuo = window.sessionStorage.getItem("addjinhuo");
        if (addjinhuo != null && addjinhuo != "" && addjinhuo != "null") {
            //注册表单验证
            $(validform());
            $("#jinhuoUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
            ...
            window.sessionStorage.removeItem('updateId');
            http("jinhuo/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    function dateTimePick() {
        var jinhuoTime = laydate.render({
            elem: '#jinhuoTime-input'
            ,type: 'datetime'
        });
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#jinhuoUuidNumber").val(ruleForm.jinhuoUuidNumber);
        $("#jinhuoNumber").val(ruleForm.jinhuoNumber);
        $("#jinhuoTime-input").val(ruleForm.jinhuoTime);
        $("#jinhuoContent").val(ruleForm.jinhuoContent);
