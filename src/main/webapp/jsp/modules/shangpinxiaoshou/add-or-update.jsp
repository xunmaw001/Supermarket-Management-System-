            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">商品销售管理</li>
                        <li class="breadcrumb-item active">编辑商品销售</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="shangpinId" name="shangpinId" type="hidden">
                                    <div class="form-group col-md-6 shangpinxiaoshouUuidNumberDiv">
                                        <label>商品销售单号</label>
                                        <input style="width: 450px" id="shangpinxiaoshouUuidNumber" name="shangpinxiaoshouUuidNumber" class="form-control"
                                               placeholder="商品销售单号">
                                    </div>
                                    <div class="form-group col-md-6 shangpinxiaoshouNumberDiv">
                                        <label>商品销售数量</label>
                                        <input style="width: 450px" id="shangpinxiaoshouNumber" name="shangpinxiaoshouNumber" class="form-control"
                                               onchange="shangpinxiaoshouNumberChickValue(this)"  placeholder="商品销售数量">
                                    </div>

                                    <%--<div class="form-group col-md-6 shangpinxiaoshouTruePriceDiv">
                                        <label>销售金额</label>
                                        <input style="width: 450px" id="shangpinxiaoshouTruePrice" name="shangpinxiaoshouTruePrice" class="form-control"
                                               onchange="shangpinxiaoshouTruePriceChickValue(this)" placeholder="销售金额">
                                    </div>--%>
                                    <div class="form-group col-md-6 shangpinxiaoshouTimeDiv">
                                        <label>商品销售时间</label>
                                        <input style="width: 450px" id="shangpinxiaoshouTime-input" name="shangpinxiaoshouTime" type="text" class="form-control layui-input">
                                    </div>
                                    <div class="form-group  col-md-6 shangpinxiaoshouContentDiv">
                                        <label>商品销售备注</label>
                                        <input id="shangpinxiaoshouContentupload" name="file" type="file">
                                        <script id="shangpinxiaoshouContentEditor" type="text/plain"
                                                style="width:100%;height:230px;"></script>
                                        <script type = "text/javascript" >
                                        //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                        //相见文档配置属于你自己的编译器
                                        var shangpinxiaoshouContentUe = UE.getEditor('shangpinxiaoshouContentEditor', {
                                            toolbars: [
                                                [
                                                    'undo', //撤销
                                                    'bold', //加粗
                                        ...
                                        <input type="hidden" name="shangpinxiaoshouContent" id="shangpinxiaoshouContent-input">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#shangpinxiaoshouContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('shangpinxiaoshouContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("shangpinxiaoshou/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addshangpinxiaoshou');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.shangpinxiaoshouContent != null && ruleForm.shangpinxiaoshouContent != 'null' && ruleForm.shangpinxiaoshouContent != '' && $("#shangpinxiaoshouContentupload").length>0) {

            var shangpinxiaoshouContentUeditor = UE.getEditor('shangpinxiaoshouContentEditor');
            shangpinxiaoshouContentUeditor.ready(function () {
                var mes = '';
                if(ruleForm.shangpinxiaoshouContent != null){
                    mes = ''+ ruleForm.shangpinxiaoshouContent;
                    // mes = mes.replace(/\n/g, "<br>");
                }
                shangpinxiaoshouContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#shangpinxiaoshouContentupload").length>0) {
            var shangpinxiaoshouContentEditor = UE.getEditor('shangpinxiaoshouContentEditor');
            if (shangpinxiaoshouContentEditor.hasContents()) {
                $('#shangpinxiaoshouContent-input').attr('value', shangpinxiaoshouContentEditor.getContent());
            }
        }
    }
    //数字检查
        <!-- 当前表的数字检查 -->
        function shangpinxiaoshouNumberChickValue(e){
        ...
        function shangpinxiaoshouTruePriceChickValue(e){
            var this_val = e.value || 0;
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                huiyuanId: "required",
                shangpinId: "required",
                shangpinxiaoshouUuidNumber: "required",
                shangpinxiaoshouNumber: "required",
                shangpinxiaoshouTruePrice: "required",
                shangpinxiaoshouTime: "required",
                shangpinxiaoshouContent: "required",
            },
            messages: {
                yuangongId: "员工不能为空",
                huiyuanId: "会员不能为空",
                shangpinId: "商品不能为空",
                shangpinxiaoshouUuidNumber: "商品销售单号不能为空",
                shangpinxiaoshouNumber: "商品销售数量不能为空",
                shangpinxiaoshouTruePrice: "销售金额不能为空",
                shangpinxiaoshouTime: "商品销售时间不能为空",
                shangpinxiaoshouContent: "商品销售备注不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addshangpinxiaoshou = window.sessionStorage.getItem("addshangpinxiaoshou");
        if (addshangpinxiaoshou != null && addshangpinxiaoshou != "" && addshangpinxiaoshou != "null") {
            //注册表单验证
            $(validform());
            $("#shangpinxiaoshouUuidNumber").val(new Date().getTime()+Math.ceil(Math.random()*10));//设置唯一号
            ...
            window.sessionStorage.removeItem('updateId');
            http("shangpinxiaoshou/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    function dateTimePick() {
        var shangpinxiaoshouTime = laydate.render({
            elem: '#shangpinxiaoshouTime-input'
            ,type: 'datetime'
        });
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#shangpinxiaoshouUuidNumber").val(ruleForm.shangpinxiaoshouUuidNumber);
        $("#shangpinxiaoshouNumber").val(ruleForm.shangpinxiaoshouNumber);
        $("#shangpinxiaoshouTruePrice").val(ruleForm.shangpinxiaoshouTruePrice);
        $("#shangpinxiaoshouTime-input").val(ruleForm.shangpinxiaoshouTime);
        $("#shangpinxiaoshouContent").val(ruleForm.shangpinxiaoshouContent);
    ...
    function huiyuanDataBind(){
        ...
        $("#huiyuanbiaoqianName").val(ruleForm.huiyuanbiaoqianName);
    ...
    function shangpinDataBind(){
        ...
        $("#shangpinDanwei").val(ruleForm.shangpinDanwei);
